...
class FFTChartBase extends ChartBase {
	...
	setupUnitTypeName(payload) {
		...
		if (
			this.currentSensorType.isBannerM25 &&
			(
...
class FFTChart extends FFTChartBase {
		...
		try {
			...
			if (requestsList && requestsList.length) {
				...
				if (axisItem && axisItem.type_key) {
					let axisType = isBannerM25
						? axisItem.ultrasound_fft_name_full
						: axisItem.type_key;
					let vertical = '';
	...
	generateAnnotationsFromFFTAnalysisRules(selectedAnalysisRules) {
		...
		selectedAnalysisRules.forEach((rule_item, idx) => {
			...
			const activeValue =
				(rule_item.option_value_id && rule_item.option_value?.value) || rule_item.custom_value;
			...
			const isValidData = !isNaN(startX) && !isNaN(stepX) && stepX > 0;
