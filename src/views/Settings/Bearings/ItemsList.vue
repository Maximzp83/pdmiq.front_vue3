...
defineOptions({
	name: 'SettingsBearingsList',
});

const itemsTableRef = ref(null);
const itemStore = useBearingsStore();
const { filters } = storeToRefs(itemStore);
const authStore = useAuthStore();
const entity = ENTITIES.Bearings;
...
const tableSettings = computed(() => {
	...
	return {
		columns: translate([
			{ prop: 'number', label: 'Number', sortable: true },
			{ prop: 'width', label: 'Width' },
			{ prop: 'outside_diameter', label: 'phrases.Outside_diameter' },
		]),
