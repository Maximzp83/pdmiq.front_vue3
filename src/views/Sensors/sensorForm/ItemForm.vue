		...
		>
			<div class="tab-container" :class="{ 'half-width': !fromAnotherInstance && !isMobile }">
				...
				<el-form-item label="MAC" prop="mac_address">
					<CustomInput v-model="formData.mac_address" placeholder="MAC" />
				</el-form-item>

				<el-form-item :label="tt('Node')" prop="port_number">
					<el-input-number v-model="formData.port_number" :min="0" />
				</el-form-item>

				<el-form-item :label="`${tt('device')} id`" prop="device_address_id">
					<CustomInput v-model="formData.device_address_id" />
				</el-form-item>

				<el-form-item :label="`${tt('sensor')} id`" prop="fft_sensor_id">
...
const dataSets = computed(() =>
	dataSetsList().filter((item) => item.controller_type === SENSOR_TYPES.BANNER),
);
...
const controllersSelectSettings = computed(() => ({
	...
	params: { max: 100, type: SENSOR_TYPES.BANNER },
}));
