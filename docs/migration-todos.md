...
- DONE (Vue2 sync): migrated latest Sensors/chart/sub-item legacy changes into already migrated Vue3 files: boolean `setIfEmpty` false normalization in `useSubItemsList`, Sensor chart one-chart navigator/scrollbar disable with x panning, `ChartsListWrapper` one-chart additional-props/header/getParams guards, and Banner M25 running-threshold parameter filtering in Sensor form; skipped whitespace-only Sensor methods change and legacy WebSocketService broadcasting auth fallback because Vue3 uses native websocket endpoint settings; `npm run build` and targeted `git diff --check` pass.
