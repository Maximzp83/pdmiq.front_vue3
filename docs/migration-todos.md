...
- DONE (coupled step): added first Vue3 `Equipments` create/edit compile scope with `ItemPage.vue`, `ItemFormWrapper.vue`, and `ItemForm.vue`; the form covers required/core fields and uploads while advanced RPM/vibration/subtype/sensor/multiview tabs remain simplified/pending; `npm run build` passes.
...
- Current next focus: check remaining forms in `src/views/SuccessDashboard` one file at a time against Vue2 originals and smoke-test ROI One Pager with real item data.
