...
- Vue2 changes from local commit `681a6b8` were synced into six migrated Assets/Machines/ProductionLines details/form files on the current branch: details edit buttons include `vertical-fluid`, navbar `showPlantName` includes plant `id`, and related form requests resolve the navbar plant before the global filter. The changed Utilities details file was skipped because no migrated Vue3 counterpart exists. Targeted ESLint, `git diff --check`, and `npm run build` pass.
- Latest uncommitted Vue2 sync completed for Controllers and Plants: `src/views/Controllers/DXMCommandsTab.vue` sends `CMD0200` directly without WebSocket setup, while other commands retain the socket lifecycle; `src/views/Plants/ItemForm.vue` no longer renders or submits `is_blocked`. English/Spanish localization changes were whitespace/key-order-only with unchanged values, so no Vue3 localization update was applied. Targeted ESLint, targeted `git diff --check`, and `npm run build` pass.
