...
- Latest Sensors/chart/sub-item Vue2 changes were synced into already migrated Vue3 files: `useSubItemsList` now normalizes boolean `setIfEmpty` false values to `0`; Sensor chart `oneChartOnly` disables navigator/scrollbar and enables x panning; Sensors `ChartsListWrapper` guards `getParamsByIds`, passes computed per-chart `additionalProps`, and hides chart header for single one-chart results; Sensor form limits Banner M25 running-threshold parameters to the updated subset. The Vue2 Sensor methods change was whitespace-only, and the legacy `WebSocketService` broadcasting auth fallback has no Vue3 equivalent because Vue3 uses native websocket endpoint settings. `npm run build` and targeted `git diff --check` pass.
- Latest Vue2 form/Requisitions changes were synced into already migrated Vue3 files: Machine and Production Line forms now support silence mode plus `silence_mode_until` date cleanup; Plant form now includes the existing-plant `is_blocked` switch for archive-permitted users; Requisitions list, Work Order print report, and ROI list Hours fields now use `execution_total_time`; Vue3 localization already had the new silence mode phrase keys. `npm run build` and targeted `git diff --check` pass.
