							...
							<div class="card">
								...
								<div class="card-content main filled">
									<ul class="dots-in-text">
										<li
											v-for="item in orderData.actualMaterials"
											:key="`acm-${item.id}`"
										>
								...
								<div class="card-content filled">
									<ul class="dots-in-text bold">
										<li>
											...
											<span
												class="value"
												v-text="`${orderData.execution_cost}$`"
											></span>
										</li>
									</ul>
								</div>
...
export default {
	components: {
		ReportValueCell: () => import('./ReportValueCell.vue')
	},
	props: {
		...
		infoItemsList1() {
			return Object.freeze([
				...
				},
				{
					...
					meta: {
						fromArray: { subProp: 'full_file_name', delimeter: ', ' }
					}
				}
			]);
		},
