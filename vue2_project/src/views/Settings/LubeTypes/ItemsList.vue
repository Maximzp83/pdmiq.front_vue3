...
export default {
	...
	computed: {
		...mapState({
			filters: state => state.lube_types.filters
		}),
		...
		itemsName: that => ({
			one: that.$t('Lube_Type'),
			mult: that.$t('Lube_Types'),
		...
		localModalSettings() {
			let componentPath = 'Settings/LubeTypes/ItemForm';
	...
	methods: {
		...mapActions({
			...
			fetch_items: 'lube_types/fetch_lube_types',
			save_item: 'lube_types/save_lube_type',
			delete_item: 'lube_types/delete_lube_types',

			set_filters: 'lube_types/set_lube_types_filters'
		})
