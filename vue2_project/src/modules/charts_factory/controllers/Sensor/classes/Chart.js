...
class SensorChartBase extends ChartBase {
			...
			xAxis: [
				{
					...
					events: {
						afterSetExtremes: e => {
							if (e.trigger == 'navigator' && this.events.navigatorChangeHandler) {
								this.events.navigatorChangeHandler({min: e.min, max: e.max, chart_id: this.chart_id});
							} else if (e.trigger == 'zoom' && this.events.navigatorChangeHandler) {
								this.events.navigatorChangeHandler({min: e.min, max: e.max, chart_id: this.chart_id});
							}
						}
					}
	...
	syncNavigatorPosition({chart_id, min, max}) {
		// console.log('syncNavigatorPosition', min, max, chart_id, this.chart_id);
		if (chart_id !== this.chart_id && this.ChartAPI) {
